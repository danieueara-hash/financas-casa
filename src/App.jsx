...
const DEF_CATS = [
  ...
  {name:"Investimentos",type:"income",color:"#F97316",emoji:"📈"},
...
const GhostBtn = ({children,onClick,full=false}) => (
  <button onClick={onClick}
    className={`flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors cursor-pointer ${full?"w-full":""}`}>
    {children}
  </button>
)
const Inp = ({label,...p}) => (
  <div className="w-full">
    ...
    <input className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-emerald-400 transition-colors bg-white" {...p}/>
  </div>
)
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({mTxns,allCats,cards,txns,month}){
  ...
  const cardUsage=cards.map(c=>({...c,used:txns.filter(t=>t.card_id===c.id&&t.date.startsWith(month)).reduce((s,t)=>s+t.amount,0)}))

  const kpis=[
    {label:"Receitas",value:income,color:"#10B981",bg:"#ECFDF5",icon:"📈"},
    {label:"Despesas",value:expense,color:"#EF4444",bg:"#FEF2F2",icon:"📉"},
    {label:"Saldo",value:bal,color:bal>=0?"#10B981":"#EF4444",bg:bal>=0?"#ECFDF5":"#FEF2F2",icon:"💰"},
        ...
        {kpis.map(k=>(
          <div key={k.label} className="bg-white rounded-2xl p-5 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="w-8 h-8 rounded-xl flex items-center justify-center text-base" style={{background:k.bg}}>{k.icon}</div>
            </div>
                ...
                {catChart.map((c,i)=>(
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600">{c.emoji} {c.name}</span>
                    </div>
              ...
              {cardUsage.map(c=>(
                <div key={c.id} className="rounded-xl p-3" style={{border:`1px solid ${c.color}30`,background:c.color+"08"}}>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">d.{c.due_day}</span>
                  </div>
                  <div className="w-full h-1.5 bg-gray-100 rounded-full mb-1">
                    <div className="h-full rounded-full" style={{width:`${Math.min(c.used/c.limit_amount*100,100)}%`,background:c.color}}/>
                  </div>
                  <div className="flex justify-between text-xs text-gray-400">
                    <span>{R(c.used)}</span><span>{R(c.limit_amount)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {items.length===0?<p className="text-sm text-gray-400 text-center py-4">Tudo em dia ✅</p>:(
              <div className="space-y-1.5" style={{maxHeight:180,overflowY:"auto"}}>
                {items.map(t=>{const cat=getCat(t.category_id);return(
                  <div key={t.id} className="flex items-center justify-between p-2 rounded-xl hover:bg-gray-50">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{cat?.emoji||"📦"}</span>
                      <div><p className="text-xs font-semibold text-gray-800">{t.description}</p><p className="text-xs text-gray-400">{Dt(t.date)}</p></div>
                    </div>
...
function Transactions({mTxns,allCats,cards,onEdit,onDelete,onToggle}){
  ...
  const rows=[...mTxns].filter(t=>filt==="all"||t.type===filt).sort((a,b)=>b.date.localeCompare(a.date))
  return(
    <div>
      <div className="flex items-center gap-3 mb-5">
        <p className="text-base font-bold text-gray-900 whitespace-nowrap">Lançamentos</p>
        <Tabs opts={[["all","Todos"],["expense","Despesas"],["income","Receitas"]]} val={filt} onChange={setFilt}/>
      </div>
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {rows.length===0?<p className="text-sm text-gray-400 text-center py-14">Nenhum lançamento neste mês</p>:(
          rows.map((t,i)=>{
            const cat=allCats.find(c=>c.id===t.category_id)
            const card=cards.find(c=>c.id===t.card_id)
            const ok=t.status==="paid"||t.status==="received"
            return(
              <div key={t.id} className={`flex items-center px-5 py-3.5 hover:bg-gray-50 group ${i<rows.length-1?"border-b border-gray-50":""}`}>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <span className="text-xs px-1.5 py-0.5 rounded-full" style={{background:ok?"rgba(16,185,129,0.1)":"rgba(245,158,11,0.1)",color:ok?"#10B981":"#F59E0B"}}>
                      {t.status==="paid"?"Pago":t.status==="received"?"Recebido":"Pendente"}
                    </span>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button title="Marcar pago" onClick={()=>onToggle(t)} className="p-1.5 rounded-lg hover:bg-emerald-50 text-gray-300 hover:text-emerald-500 cursor-pointer">✓</button>
...
// ── CARDS SCREEN ──────────────────────────────────────────────────────────────
function CardsScreen({cards,txns,month,allCats,onAdd,onEdit,onDelete}){
  const [active,setActive]=useState(cards[0]?.id||null)
  const stmnt=txns.filter(t=>t.card_id===active&&t.date.startsWith(month))
  ...
  return(
    <div className="space-y-5">
      ...
      {cards.length===0?(
        <div className="bg-white rounded-2xl p-14 text-center shadow-sm"><p className="text-4xl mb-2">💳</p><p className="text-sm text-gray-400">Nenhum cartão cadastrado</p></div>
      ):(
        <>
                ...
                <div key={c.id} onClick={()=>setActive(c.id)} className="cursor-pointer rounded-2xl p-5 shadow-sm hover:shadow-md transition-all"
                  ...
                  <div className="flex justify-between items-start mb-5">
                    ...
                    <div className="flex gap-1">
                      <button onClick={e=>{e.stopPropagation();onEdit(c)}} className="text-xs opacity-60 hover:opacity-100 cursor-pointer" style={{color:sel?"white":"#9CA3AF"}}>✏️</button>
                      <button onClick={e=>{e.stopPropagation();onDelete(c.id)}} className="text-xs opacity-60 hover:opacity-100 cursor-pointer" style={{color:sel?"white":"#9CA3AF"}}>🗑️</button>
                    </div>
                  </div>
  ...
  return(
    <div>
      ...
      {cats[tab].length===0?(
        ...
        <div className="grid grid-cols-3 gap-4">
          {cats[tab].map(c=>(
            <div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow group">
              <div className="flex items-center gap-3">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={()=>onEdit(c,tab)} className="text-sm text-blue-300 hover:text-blue-500 cursor-pointer p-1">✏️</button>
                <button onClick={()=>onDelete(c.id,tab)} className="text-sm text-red-300 hover:text-red-500 cursor-pointer p-1">🗑️</button>
              </div>
            </div>
          ))}
        </div>
...
function Reports({txns,cats,month}){
  ...
  const chartData=useMemo(()=>Array.from({length:6},(_,i)=>{
    ...
    const inc=txns.filter(t=>t.date.startsWith(m)&&t.type==="income"&&t.status==="received").reduce((s,t)=>s+t.amount,0)
    const exp=txns.filter(t=>t.date.startsWith(m)&&t.type==="expense"&&t.status==="paid").reduce((s,t)=>s+t.amount,0)
    return{label:MONTHS[mo-1].slice(0,3),inc,exp,bal:inc-exp}
  }),[txns,month])
  const catData=useMemo(()=>{
    const g={}
    txns.filter(t=>t.date.startsWith(month)&&t.type==="expense"&&t.status==="paid").forEach(t=>{g[t.category_id]=(g[t.category_id]||0)+t.amount})
              ...
              {catData.map((c,i)=>(
                <div key={i}>
                  ...
                  <div className="w-full h-1.5 rounded-full bg-gray-100">
                    <div className="h-full rounded-full" style={{width:`${total?c.value/total*100:0}%`,background:c.color}}/>
                  </div>
                </div>
              ))}
            </div>
            ...
            {chartData.map((m,i)=>(
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-100 overflow-hidden">
                  <div className="h-full rounded-full" style={{width:`${Math.min(Math.abs(m.bal)/7000*100,100)}%`,background:m.bal>=0?"#10B981":"#EF4444"}}/>
                </div>
...
function AiTip({tip,loading,onClose}){
  return(
    <div className="fixed bottom-6 right-6 z-50 w-80 bg-white rounded-2xl shadow-2xl border border-emerald-100 overflow-hidden">
      ...
      <div className="p-4">
        {loading?<p className="text-sm text-gray-400 flex items-center gap-2"><span className="animate-spin inline-block">⏳</span> Analisando suas finanças...</p>
...
export default function App(){
  ...
  // Forms
  const blankTx={type:"expense",description:"",amount:"",category_id:"",date:today(),status:"pending",card_id:""}
  ...
  const allCats=useMemo(()=>[...cats.expense,...cats.income],[cats])
  const mTxns=useMemo(()=>txns.filter(t=>t.date&&t.date.startsWith(month)),[txns,month])
  ...
  const loadData=useCallback(async userId=>{
    ...
    if(catsData.length===0){
      const toInsert=DEF_CATS.map(c=>({...c,user_id:userId}))
      const {data:seeded}=await supabase.from("categories").insert(toInsert).select()
      catsData=seeded||[]
    }
    setCats({
      expense:catsData.filter(c=>c.type==="expense"),
      income:catsData.filter(c=>c.type==="income"),
    })
  ...
  const saveTx=async()=>{
    ...
    const payload={
      user_id:uid,
      type:txF.type,
      description:txF.description,
      amount:parseFloat(txF.amount),
      category_id:txF.category_id,
      date:txF.date,
      status:txF.status,
  ...
  const saveCard=async()=>{
    ...
    const payload={user_id:uid,name:cardF.name,color:cardF.color,limit_amount:parseFloat(cardF.limit_amount),closing_day:parseInt(cardF.closing_day)||1,due_day:parseInt(cardF.due_day)||10}
    if(editCardId){
  ...
  const saveCat=async()=>{
    if(!catF.name) return
    const payload={user_id:uid,name:catF.name,color:catF.color,emoji:catF.emoji,type:catF.type}
    const t=catF.type
    if(editCatId){
  ...
  const triggerAI=async tx=>{
    ...
    const catTotal=txns.filter(t=>t.date?.startsWith(month)&&t.category_id===tx.category_id&&t.status==="paid").reduce((s,t)=>s+t.amount,0)+tx.amount
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          system:"Você é um consultor financeiro pessoal simpático e direto. Responda em português do Brasil com 2-3 frases curtas e práticas. Sem markdown nem asteriscos.",
          messages:[{role:"user",content:`Usuário registrou uma ${tx.type==="expense"?"despesa":"receita"} de ${R(tx.amount)} em "${cat?.name}". Receita do mês: ${R(inc)}. Despesas pagas: ${R(exp)}. Total em ${cat?.name}: ${R(catTotal)}. Dê uma dica financeira curta e relevante.`}]
        })
      })
      const data=await res.json()
      setAiTip(data.content?.[0]?.text||"Continue acompanhando suas finanças!")
    }catch{ setAiTip("Dica: Registre todos os gastos para ter controle total das suas finanças!") }
    setAiLoad(false)
  }
  ...
  return(
    <div className="min-h-screen flex" style={{background:"#F8FAFC"}}>
      ...
      <div className="w-56 flex-shrink-0 flex flex-col" style={{background:"#0F172A"}}>
        ...
        <div className="p-3">
          <button onClick={()=>supabase.auth.signOut()} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer">
            <span>🚪</span>Sair
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 p-6 overflow-auto">
          {dataLoading?(
            ...
            <>
              {scr==="dashboard"&&<Dashboard mTxns={mTxns} allCats={allCats} cards={cards} txns={txns} month={month}/>}
              ...
              {scr==="cards"&&<CardsScreen cards={cards} txns={txns} month={month} allCats={allCats} onAdd={openAddCard} onEdit={openEditCard} onDelete={deleteCard}/>}
      ...
      {cardMod&&(
        <Modal title={editCardId?"Editar Cartão":"Novo Cartão"} onClose={()=>setCardMod(false)}>
          <Inp label="Nome" placeholder="Ex: Nubank, Inter..." value={cardF.name} onChange={e=>setCardF({...cardF,name:e.target.value})}/>
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {["#8B5CF6","#F97316","#EF4444","#10B981","#3B82F6","#EC4899","#F59E0B","#06B6D4"].map(c=>(
                <button key={c} onClick={()=>setCardF({...cardF,color:c})} className="w-7 h-7 rounded-full border-2 cursor-pointer transition-transform hover:scale-110"
      ...
      {catMod&&(
        <Modal title={editCatId?"Editar Categoria":"Nova Categoria"} onClose={()=>setCatMod(false)}>
          ...
          <Inp label="Nome" placeholder="Ex: Pets, Streaming..." value={catF.name} onChange={e=>setCatF({...catF,name:e.target.value})}/>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap mt-1">
                {["#10B981","#F59E0B","#EF4444","#3B82F6","#8B5CF6","#EC4899","#06B6D4","#F97316"].map(c=>(
                  <button key={c} onClick={()=>setCatF({...catF,color:c})} className="w-6 h-6 rounded-full cursor-pointer border-2 transition-transform hover:scale-110"
